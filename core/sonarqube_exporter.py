...
class SonarExporter:
    ...
    def _request(self, endpoint):
        _url = "{}/{}".format(self.base_url, endpoint)
        print(_url)
        headers = {'Accept': 'application/json', 'Content-Type': 'application/json'}
        req = requests.get(_url, auth=(self.user, self.password),  headers=headers)
...
def get_all_projects_with_metrics():
    ...
    for project in all_projects['components']:
        p = Project(identifier=project['name'], key=project['key'])
        p.name = project['name']
        p.organization = project['qualifier']
